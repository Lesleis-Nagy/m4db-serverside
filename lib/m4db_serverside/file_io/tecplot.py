...
regex_var_location = re.compile(r"VARLOCATION\s*=\s*\(([A-Z0-9-,\[\]=]+)\)")
regex_var_share_list = re.compile(r"VARSHARELIST\s*=\s*\(([0-9-,\[\]=]+)\)")
regex_connectivity_share_zone = re.compile(r"CONNECTIVITYSHAREZONE\s*=\s*([0-9]+)")
regex_floats = re.compile(r"(-?[0-9]+\.[0-9]+E[+-]?[0-9]{2}(\s+-?[0-9]+\.[0-9]+E[+-]?[0-9]{2})*)")
...
def read_zone(tecplot_file, zone_metadata, index_offset=-1):
    # Parse vertex data
